...
public class NopperabouBehaviour : MonoBehaviour
{
    // Velocidad
    private float speed = 1.5f;

    // Ultima posición conocida del jugador
    Vector3 destinationHeard = Vector3.zero;
    ...
    void moveToPlayer()
    {
        ...
        if (door)
        {
            _agent.SetDestination(destinationHeard);

            if (Vector3.Distance(transform.position, destinationHeard) <= _agent.stoppingDistance)
                task.Succeed();
            else
                anim.SetBool("attack", false);
        }
        else if (_agent.destination != destinationHeard)
        {
            destinationHeard = player.transform.position;

            NavMeshHit hit;
            int aux = Random.Range(0, waypoints.Count);
            NavMesh.SamplePosition(destinationHeard, out hit, 0.5f, NavMesh.AllAreas);
            _agent.destination = hit.position;
